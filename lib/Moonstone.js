  ...
  wsEvent(packet) {
    switch (packet.op) {
      ...
      case OPCodes.USER_JOIN_ROOM: {
        ...
        if (room) {
          const user = room.users.add(new RoomUser(packet.d.user));
          room.updateVoiceStates(packet.d);
      ...
      case OPCodes.ACTIVE_SPEAKER_CHANGE: {
        var room = this.rooms.get(packet.d.roomId);
        if (room) {
          const updates = room.updateVoiceStates(packet.d, true);
      ...
      case OPCodes.DEAFEN_CHANGED: {
        ...
        if (room) {
          var user = room.users.get(packet.d.userId);
          user.deafened = packet.d.value;
          this.emit("deafenChange", user, room);
        }
        break;
      }
      case OPCodes.NEW_CHAT_MSG: {
        var user = this.currentRoom.users.get(packet.d.userId);
        if (user) {
          // TODO proper type
          this.emit("newChatMsg", packet.d.msg, user, this.currentRoom);
        } else {
          console.log("Received message from unknown user"); // TODO
        }
        break;
      }
