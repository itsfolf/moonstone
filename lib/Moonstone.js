...
class Moonstone extends EventEmitter {
  ...
  wsEvent(packet) {
    switch (packet.op) {
      ...
      case OPCodes.USER_JOIN_ROOM: {
        ...
        if (room) {
          const user = room.users.add(new RoomUser(packet.d.user, room, this));
      ...
      case OPCodes.ACTIVE_SPEAKER_CHANGE: {
        ...
        if (room) {
          ...
          updates[2].forEach((user) => this.emit("deafenChange", user, room));
        }
        break;
      }
  ...
  createRoom({ name, description, privacy }) {
    return new Promise((resolve, reject) => {
      this.waitForRef(
